...
@section('content')
    ...
    <div class="cardnewPage">
        <div class="container">
            <form class="cardForm" action="/listing/{{$listing_id}}/card" accept-charset="UTF-8" data-remote="true" method="post"/>
                {{csrf_field()}}
                <input type="hidden" value="{{$listing_id}}" name="listing_id">
                <div class="cardnewForm_title">
                    <label for="caard_title">タイトル</label>
                    <input type="text" name="card_title" value="{{old('card_title')}}" placeholder="カード名" class="form-control" autofocus="autofocus">
                </div>
                <div class="cardnewForm_memo">
                    <label for="card_memo">メモ</label>
                    <textarea autofocus="autofocus" class="form-control" placeholder="詳細" name="card_memo">{{ old('card_memo') }}</textarea>
